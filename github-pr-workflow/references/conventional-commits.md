...
## Avoid inline `-m` for messages containing `!`, backticks, or `$`

A `!` anywhere in a double-quoted `git commit -m "..."` argument (e.g. the breaking-change marker `feat!:` below) triggers bash history expansion (`bash: unrecognized history modifier`), garbling or aborting the commit. Write the message to a file and use `-F`:

```bash
cat > /tmp/commit-msg.txt <<'EOF'
feat!: add JWT-based user authentication

BREAKING CHANGE: uses `backticks` and ! freely — heredoc needs no escaping
EOF
