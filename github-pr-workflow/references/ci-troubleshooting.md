...
curl -sL -H "Authorization: token $GITHUB_TOKEN" \
  ...
  -o /tmp/ci-logs.zip && unzip -o /tmp/ci-logs.zip -d /tmp/ci-logs && cat /tmp/ci-logs/*.txt
```
...
Once the fix is identified, push it and re-check status using SKILL.md § 5 Steps 2-3 (`git add`/`commit`/`push`, then re-run the Section 4 status check) — same commands, not repeated here.
